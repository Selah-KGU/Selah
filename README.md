...
Selah は関西学院大学の教務システム **KWIC** と学習管理システム **Luna (LMS)**、さらに大学の **Microsoft 365 メール**のデータを統合し、ネイティブデスクトップアプリとして提供します。ブラウザを開かずに、授業・課題・成績・お知らせ・メールをすばやく確認できます。macOS / Windows の両プラットフォームに対応しています。
...
- 天気情報の表示（Open-Meteo API による現在地の気温・天候）
...
- **カレンダー同期** — macOS の Apple カレンダーまたは Google カレンダーへ時間割を自動エクスポート（変更検知による差分同期）
...
- **AI 学習アシスト** — OpenAI / Gemini API を使い、時間割・シラバス・締切を踏まえた学習計画とタスクごとの取り組み方ガイドを生成（結果は 6 時間キャッシュ）
...
- **SSO 認証 (Cookie Bridge 方式)** — 内蔵 WebView (WKWebView / WebView2) で関学 SSO のログインフォームを表示し、取得した Cookie を Rust 側 HTTP クライアントへ橋渡し。KG-Course・Luna・KWIC の 3 系統を一度のログインで認証
...
- **セキュリティ** — 保存される Cookie・セッション・トークンファイルには Unix 上で制限付きパーミッション (0600) を設定
...
| 認証 | Cookie Bridge (WKWebView / WebView2 経由) |
| キャッシュ | SQLite (WAL モード) |
| ビルドツール | [Vite](https://vitejs.dev/) |
| パッケージ | macOS: DMG / .app (universal binary) ／ Windows: NSIS インストーラー |
...
| OpenAI / Google Gemini API | AI アシスト機能 | ユーザーが API キーを設定した場合のみ |
...
AI アシスト機能（履修分析・学習計画・通知サマリー）はユーザー自身が設定した API キーで直接 OpenAI または Google Gemini に接続します。AI への送信内容は時間割・シラバス・成績などの学術データに限られ、開発者のサーバーを経由することはありません。API キーは OS のキーチェーンに安全に保存されます。
